...
| 3 | 1.6초 라이브 조건 평가 생성 | ✅ | `eval_traj_split.py`가 1.6s·4.8s 지평선 ADE/FDE를 scene-level CI로 산출(실데이터 재실행 완료). PR #19 |
| 4 | 전원 batch 예측 + 미래 위험 중재 (P0-5) | ✅ | `trajectory/risk.py` + `/predict` 배치 + sim `MPRED` + worst→제어. GT+검출 모드. PR #16·#17 |
| 5 | 독립 test split + scene-level CI (P0-1) | ✅ | seed 단위 manifest split(84/18/18), `bootstrap.py` scene-level 95% CI, val=선택/test=1회. PR #19 |
| 6 | 실제 detector-track E2E 평가 (P0-2) | ✅ | overhead-person 클립에서 GT-트랙 vs YOLO+ByteTrack. 병목=검출 커버리지(30%). PR #18 |
| 7 | API/browser CI (P0-4) | 🟡 | 브라우저 명세 `tests/browser/*` 작성됨. **Playwright 러너 미배선 + GitHub Actions CI 미구성**(← 다음 작업) |
| 8 | 발표 모드 + 성능 계측 (P1-3) | ⬜ | N=1/3/5/10 지연 실측 포함 |
| 9 | 모델 artifact revision 고정 + 온디바이스 (P1-1/P1-2) | ⬜ | `model*.pt`이 HF-managed(gitignore)라 revision 해시 고정 필요 |
| 10 | 라이브 안전 성능 계측 (P0-3) | 🟡 | recall/precision(=missed entry·false hold 근사)은 산출. **lead time·stop duration KPI, 라이브 seed 비교, 고정 seed 회귀·실패 사례** 남음 |

### 백본·학습 비교 (감사 밖 · 이번 세션 추가)
- ✅ **Transformer 백본 vs LSTM**(PR #19): 백본만 교체한 공정 A/B. 정확도·안전 recall에서 Transformer 우위.
- 🟢 **이산 잠재 CVAE(Trajectron++식)**(PR #20, 리뷰 대기): 3-way 비교. 정확도는 Transformer 동급, **안전 recall/precision 트레이드오프에서 우위**(같은 recall에 헛정지↓).

### P0-5 내부 잔여 (다인원)
- ✅ **검출 모드 관측**(PR #17): `MPRED` 관측을 검출 모드에선 `MIL.tracks`로 분기 → 검출 모드 다인원 제어 활성.
- ⬜ **실모델 E2E**: 브라우저 검증은 mock `/predict` 사용(백엔드 로직은 pytest). 실제 LSTM 물린 `detect_server` 배치 E2E + N=1/3/5/10 지연 실측.
- ⬜ **3단계(상호작용 모델)**: 독립 배치가 상호작용을 반복적으로 놓친다는 측정 근거 생길 때만.

### 권장 다음 순서 (2차)
1. **P0-4 API/browser CI** → 지금까지 쌓은 걸 회귀로 잠금(이후 작업 안전).
2. **P0-3 라이브 안전 KPI**(lead time·stop duration·실패 사례) → "안전 성능 입증" 핵심.
3. **P0-5 실모델 E2E + 지연 실측** · **P1-1 모델 revision 고정**.

> 설계 상세: [multi-person](../specs/2026-08-24-multi-person-prediction-design.md) · [P0-1 split·CI](../specs/2026-08-24-traj-split-ci-design.md) · [P0-2 detector-track](../specs/2026-08-24-detection-track-eval-design.md) · [백본 비교](../specs/2026-08-24-transformer-backbone-compare-design.md) · [CVAE](../specs/2026-08-24-cvae-discrete-latent-design.md)
