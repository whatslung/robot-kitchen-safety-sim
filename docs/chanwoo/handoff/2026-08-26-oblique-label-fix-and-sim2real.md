...
약해서(0.125)+상관된 미검출 때문. 커버리지는 100%(사각지대 없음). **실배포는 실사 단일캠 0.97이라
4캠 융합이면 ≈1.0** — 융합 자체는 유효, 절대값은 검출기 성능이 좌우.
...
학습형(Transformer/LSTM)이 물리 baseline 압도. Transformer가 1.6s 후 위치를 ~12cm, 4.8s 후 38cm로
예측하고 로봇존 진입을 recall 0.73으로 조기 감지. **검출기 없이 sim GT 트랙만으로** 예측·안전 로직이
깨끗이 검증됨 — sim이 검출 학습용이 아니라 **예측/안전 검증 도구**로서 값어치를 내는 지점.

| 축 | 어디서 검증 | 결과 |
|---|---|---|
| 검출 | 실사 chef1 | 0.97 |
| 4캠 융합 | sim 실측 | 단일→융합 3.3배 (실사면 ≈1.0) |
