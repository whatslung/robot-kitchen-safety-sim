...
- **PR #25는 필요하다.** 사선 카메라 person 라벨의 부풀림·팬텀·미라벨과 RGB–GT 좌표
  어긋남을 고쳤고, 재현 가능한 분할 전 원본 생성기를 제공한다.
...
### 7.3 같은 날 다른 로컬 실행에서 나온 예비 수치

아래 값은 기존 요약 문서에 기록됐지만 데이터셋·가중치·실행 스크립트와 hash가 모두 커밋되지
않아 이번 PR만으로 독립 재현할 수 없다. 방향 확인용으로만 남긴다.

| 측정 | 예비 결과 |
|---|---|
| 660장(110장면×6캠) person, YOLO11n@960 | recall 0.344, precision 0.513, mAP50 0.356 |
| 같은 데이터, YOLO11s@1280 | recall 0.409, precision 0.518, mAP50 0.433 |
| 14장면·person 32의 어느-카메라든 검출 비율 | 11s@1280, conf 0.10에서 30/32 = 0.938 |

`0.938`은 **BEV 융합 정확도나 ByteTrack recall이 아니다.** 같은 사람을 여러 카메라에서 전역 ID로
정확히 합쳤는지, 시간축에서 ID를 유지했는지, false positive가 얼마인지는 이 값으로 알 수 없다.
또 옛 검출기와 비교할 때 라벨 외에 모델 크기·해상도·confidence도 함께 바뀌었으므로 개선 전체를
라벨 수정 하나의 효과로 돌리면 안 된다.

기존 문서의 chef1 real-only `0.970`, sim-only `0.048`, real+sim `0.961`, Transformer ADE `0.12m`,
안전진입 recall `0.73`도 동일하게 과거 로컬 예비 결과다. split manifest·가중치 hash·원본 평가
결과를 다시 보존하기 전에는 실배포나 안전 성능의 확정 근거로 사용하지 않는다.
