...
6. **최악 로밍 범위 확정**: seed별 사람 z 분포가 극단적(combo2 사방·combo3 전부 남쪽). 여러 seed로 로밍 bbox를 측정해 카메라 배치를 그 최악 기준으로 고정.
7. **실제 조리실 나디르 영상**으로 도메인 검증(현재 합성 test만).
8. (조건부) 후드 아래 안전-핵심 구역은 사선/서브후드 카메라와 조합.
