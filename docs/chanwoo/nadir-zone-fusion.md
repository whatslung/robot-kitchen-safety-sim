...
### 5-13. 6대 검증 — 반경대별 커버리지 (`train/combo_radial_eval.py`)
6대(2열×3행, x중심 ±2.3·z중심 −3.4/0/+3.4, span 5.8×4.6, dataset/combo6) 생성 후 로봇(원점) 기준 반경대별 월드 융합 recall 측정. 4대(combo2/3)와 비교.
| 반경대 | combo2 4대(분산) | combo3 4대(남쪽집중) | **combo6 6대** |
|---|---|---|---|
| 정지 <3.1m | 0.91 | 1.00 | 0.96 |
| **감속 3.1~3.9m** | 1.00 | **0.74 ⚠️** | **1.00** |
| 접근 3.9~5.0m | 1.00 | 0.96 | **1.00** |

- **4대의 문제 = 커버리지 강건성**. 사람이 분산되면(combo2) 4대로 충분하나, **한쪽으로 몰리면(combo3 전부 남쪽) 그쪽 2대만 유효 → 감속링 사람의 26%를 놓침(0.74)** — 감속을 걸어야 할 사람을 못 보는 안전 실패.
- **6대는 사람이 어디로 몰리든 감속·접근링 100% 커버**. 감속링(반경 3.9m)은 4대 사각형(남쪽 −3.3)으론 남쪽 몰림 때 못 덮지만, 6대(남쪽 행 −3.4 → −5.7)는 전 방향 링을 덮음.
- 예측 리드타임: combo6 감속링 진입자, 진입 전 8프레임 추적충실도 1.000(표본 작음).
- **결론: 예측/회피(최악 기준) 목적엔 6대가 정당**. 4대는 평균적으론 되지만 사람 몰림 시 안전링에 구멍.

### 5-14. 여러 seed 재검증 — 기하 커버리지 (24 seed, 6630 샘플)
5-13이 단일 seed·짧은 클립이라, combo3의 감속링 0.74가 우연인지 확인. combo3 실패는 **검출이 아니라 커버리지 실패**(사람 몰린 방향에 카메라 없음)였으므로, 이미지 생성 없이 **여러 seed에서 사람 위치만 측정→배치별 기하 커버리지**로 싸게 재검증(24 seed = 9+15, 6630 샘플). "커버"=≥1대 시야 안, "검출가능"=프레임 가장자리 0.3m 안쪽(sliver 아님).
| 반경대 | 4대 커버 | 4대 검출가능 | 6대 커버 | 6대 검출가능 |
|---|---|---|---|---|
| 정지 <3.1m | 1.00 | 1.00 | 1.00 | 1.00 |
| 감속 3.1~3.9m | 0.95 | **0.78** | **1.00** | **1.00** |
| 접근 3.9~5.0m | **0.64** | **0.60** | **1.00** | **0.98** |

- **combo3의 0.74는 우연이 아니었다**: 24 seed로도 **4대는 접근링의 36%가 사각**, 감속링도 22%가 가장자리 sliver(검출 불안정).
- **6대는 감속링 완벽(1.00/1.00), 접근링 커버 1.00·검출가능 0.98** — 사람 몰림 방향과 무관하게 robust.
- 이 기하 커버리지 + combo6의 검출-내-커버리지(~0.96~1.0, 5-13) = "6대는 링을 덮고(기하) 덮인 곳에서 검출됨" 두 축 성립. **6대 정당화 확정.**
- 방법 주의: 위치는 randomizeScene+extrasUpdate 로밍으로 샘플(실 사람 동선의 근사). 검출·월드융합 recall은 5-13(combo6) 참조.

### 5-15. 풀 파이프라인 — 나디르 추적 → 궤적 예측 → 안전 (4대 vs 6대, `train/nadir_predict_eval.py`)
검출·추적을 넘어 **움직임 예측/회피 목적**까지 end-to-end로 연결. 6대 긴 클립(combolong, dt 0.4s, 현실 보행 1.2m/s) 검출→월드→추적 → obs8/pred12 윈도우 → **CV·칼만·LSTM·Transformer**(LSTM/Transformer는 HF `chanubc/human-move-*`) → ADE/FDE + 위험진입(로봇 원점, 정지링 3.1m). 4대는 6대 검출을 **4대 커버리지로 필터**해 동일 궤적을 4대가 관측한 것으로 도출(관측 결측만 달라짐 → 차이=카메라 수의 값).

**관측 결측(obs8 중): 4대 1.47 vs 6대 0.16 프레임/윈도우** (4대가 18% 결측).
| 구성 | 예측기 | ADE(m) | FDE(m) | **진입 recall** | 진입 prec |
|---|---|---|---|---|---|
| 4대 | CV | 1.97 | 4.16 | 0.571 (FN48) | 0.955 |
| 4대 | Kalman | 1.75 | 3.82 | 0.696 | 0.757 |
| 4대 | **LSTM** | 1.88 | 3.20 | **0.929** (FN8) | 0.671 |
| 4대 | **Transformer** | 1.89 | 3.24 | 0.920 | 0.669 |
| 6대 | CV | **1.64** | 3.51 | 0.527 | 0.908 |
| 6대 | Kalman | **1.42** | 3.16 | 0.643 | 0.581 |
| 6대 | LSTM | 1.69 | **2.93** | 0.929 | 0.594 |
| 6대 | Transformer | 1.70 | 2.97 | 0.893 | 0.602 |

**핵심 결과:**
1. **안전 지표(진입 recall = 위험구역 진입자 예측)에서 학습 모델이 CV를 압도**: LSTM/Transformer **0.93** vs CV **0.53**. CV는 위험 진입 48건을 놓침(직선 예측이라 곡선 접근을 못 봄), LSTM은 8건만. 안전에선 "놓친 진입=충돌"이라 recall이 핵심 → **학습 모델이 CV가 못 잡는 걸 잡음.** 오경보 증가(prec↓)는 안전상 감수할 트레이드.
2. **6대가 모든 예측기에서 ADE/FDE 개선**(예: CV ADE 1.97→1.64, Kalman 1.75→1.42, LSTM FDE 3.20→2.93): 6대가 관측 결측을 1.47→0.16으로 줄여 더 깨끗한 obs → 더 정확한 예측. **카메라 수의 값이 예측까지 이어짐.**
3. **ADE(위치정확도)**에선 이 인공 동선(무작위 제어보행)이 학습 분포 밖이라 CV가 비슷/우위. 그러나 **학습 모델의 자기 분포(island 테스트셋 18씬 5895윈도우)에선 확실히 우위**: Transformer **0.682m** · LSTM 0.707m vs CV 1.103m · Kalman 0.999m (**36~38% 우수**). → 위치정확도까지 살리려면 **주방 씬 궤적으로 재학습** 필요(데이터·파이프라인 존재: `dataset/trajectories/`, `train/train_traj_*`).

**한계**: 동선이 인공(제어보행)이라 학습 모델 위치정확도는 저평가(재학습 전제). 단일 씬·로봇 원점 가정·AU=m 가정. 그래도 (a) 6대→관측깨끗→예측↑, (b) 안전 recall에서 학습 모델 우위, (c) 학습 모델의 on-distribution 우위는 견고한 신호.

**렌더 노트**: 대량 캡처는 브라우저가 **최전면 포커스**일 때만 풀스피드(rAF). 백그라운드 탭은 rAF·setTimeout이 스로틀돼 캡처가 10배 느려짐. 사람 이동은 rAF 없이도 위치 직접 제어(제어보행)로 구동, 캡처(`scene.render`+readPixels)만 포커스 필요.
...
- **커버리지·카메라 수**(5-11~5-14): 감시 목적=예측/회피 → ±5m 필요 → **6대(2열×3행)**. 4대는 사람 몰림 시 감속링 26% 놓침(24 seed 재검증). 6대는 안전링 robust.
- **풀 파이프라인**(5-15): 추적→예측→안전 end-to-end. 6대가 관측 결측↓→ADE/FDE↑. **안전(진입 recall)은 학습 모델(LSTM/Transformer 0.93)이 CV(0.53) 압도** — CV는 곡선 접근을 못 봐 위험 진입을 절반 놓침. 학습 모델은 island 자기 분포에서 CV 대비 38%↑(재학습 시 주방에서도 우위 기대).
...
8. ✅ **6대 반경대별 검증 (완료, 5-13)**: 감속·접근링 100% 커버. 4대는 사람 몰림 시 감속링 0.74로 구멍 → 6대의 강건성 확인.
9. ✅ **여러 seed 6대 재검증 (완료, 5-14)**: 24 seed 기하 커버리지 — 4대는 접근링 36% 사각, 6대는 감속·접근링 1.00. 6대 정당화 확정.
10. ✅ **풀 파이프라인 4대 vs 6대 (완료, 5-15)**: 추적→예측→안전. 6대가 관측 결측 1.47→0.16으로 줄여 ADE/FDE↑. 안전(진입 recall)은 학습 모델(0.93)이 CV(0.53) 압도.
11. **주방 씬 궤적으로 예측기 재학습**: 학습 모델이 island(자기 분포)에선 CV 대비 38%↑이나 나디르 주방 씬은 분포 밖 → 나디르 궤적 캡처로 재학습하면 위치정확도까지 우위 기대.
12. **AU→m 스케일 실측 보정**(SCALE.mPerAU 미검증 ±25%) — 안전거리 절대값의 전제.
...
- `train/combo_radial_eval.py` — 반경대별 커버리지(정지/감속/접근링) (5-13)
- `train/nadir_predict_eval.py` — 풀 파이프라인: 추적→궤적예측(CV/칼만/LSTM/Transformer)→ADE/FDE·위험진입, 4대 vs 6대 (5-15)
