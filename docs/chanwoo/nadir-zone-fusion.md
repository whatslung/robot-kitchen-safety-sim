...
3. **실배포 파이프라인화**: ByteTrack을 '저신뢰 검출 유지 + 빈 프레임 coast + id 부여' 모드로 묶어 recall 우선 운영점 확정. coast는 K로 제한(recall↔precision 트레이드).
4. **실제 조리실 나디르 영상**으로 도메인 검증(현재 합성 test만).
5. (조건부) 후드 아래 안전-핵심 구역은 사선/서브후드 카메라와 조합.
