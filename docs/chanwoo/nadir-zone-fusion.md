...
5. **6대(2열×3행)로 확대**: 4대는 로밍영역(≈8×8.5m) z축 커버리지 부족(단일 0.87~0.90). 세로 한 행 추가로 커버리지+겹침↑ → 월드 연결·상한 더 높일 여지.
6. **실제 조리실 나디르 영상**으로 도메인 검증(현재 합성 test만).
7. (조건부) 후드 아래 안전-핵심 구역은 사선/서브후드 카메라와 조합.
